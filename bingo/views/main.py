...
from bingo.models import BingoDailyRecord, BingoUser, Notification, User
from bingo.pattern_choice import GAME_PATTERN_CHOICES
import traceback

from offline_app.helper import has_valid_device, has_valid_location, is_user_offline
...
def main(request):
    ...
    try:
        if is_offline_user and (not has_valid_location(user) or not has_valid_device(user)):
