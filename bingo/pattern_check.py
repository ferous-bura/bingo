...
def check_bingo(transaction, submitted_cartella, call_number, game_pattern):
    ...
    try:
        ...
        pattern_checkers = {
            ...
            '4_middle':is_middle_four,
            'inner_and_outer':is_inner_and_outer,

            'three_lines': is_three_lines,
            'four_lines':is_four_lines,
            ...
            'any_2_lines':is_any_2_lines,
            'any_3_lines':is_any_3_lines
        }
...
def is_default_pattern(cartella, result):
    ...
    try:
        ...
        one_diagonal_check = any(all(num in result or (idx == 12 and num == cartella[12]) for idx, num in enumerate(diag)) 
...
def any_two_pattern(cartella, result):
    ...
    try:
        ...
        two_lines_check = sum(all(num in result or (idx == 12 and num == cartella[12]) for idx, num in enumerate(row)) 
                              for row in rows) >= 2 or \
                           sum(all(num in result or (idx == 12 and num == cartella[12]) for idx, num in enumerate(col)) 
        ...
        two_diagonal_x_check = all(all(num in result or (idx == 12 and num == cartella[12]) for idx, num in enumerate(diag)) 
                                   for diag in diagonals)
        
        return two_lines_check or two_diagonal_x_check
    except Exception as e:
...
def is_full_house(cartella, result):
    ...
    try:
        ...
        return all(num in result for idx, num in enumerate(cartella) if idx != 12)  # Ignore middle space
    except Exception as e:
...
def is_two_lines(cartella, result):
    ...
    try:
        ...
        return sum(all(num in result or (idx == 12 and num == cartella[12]) for idx, num in enumerate(row)) for row in rows) >= 2
    except Exception as e:
...
def is_one_line(cartella, result):
    ...
    try:
        ...
        return any(all(num in result or (idx == 12 and num == cartella[12]) for idx, num in enumerate(row)) for row in rows)
    except Exception as e:
...
def is_three_lines(cartella, result):
    ...
    try:
        ...
        return sum(all(num in result or (idx == 12 and num == cartella[12]) for idx, num in enumerate(row)) for row in rows) >= 3
    except Exception as e:
...
def is_four_lines(cartella, result):
    ...
    try:
        ...
        return sum(all(num in result or (idx == 12 and num == cartella[12]) for idx, num in enumerate(row)) for row in rows) >= 4
    except Exception as e:
...
def is_any_vertical(cartella, result):
    ...
    try:
        ...
        return any(all(num in result or (idx == 12 and num == cartella[12]) for idx, num in enumerate(col)) for col in columns)
    except Exception as e:
...
def is_any_2_vertical(cartella, result):
    ...
    try:
        ...
        return sum(all(num in result or (idx == 12 and num == cartella[12]) for idx, num in enumerate(col)) for col in columns) >= 2
    except Exception as e:
...
def is_any_diagonal(cartella, result):
    ...
    try:
        ...
        return any(all(num in result or (idx == 12 and num == cartella[12]) for idx, num in enumerate(diag)) for diag in diagonals)
    except Exception as e:
...
def is_any_1_corner(cartella, result):
    ...
    try:
        ...
        return any(num in result or (idx == 12 and num == cartella[12]) for idx, num in enumerate(corners))
    except Exception as e:
...
def is_any_2_corner(cartella, result):
    ...
    try:
        ...
        return sum(num in result or (idx == 12 and num == cartella[12]) for idx, num in enumerate(corners)) >= 2
    except Exception as e:
...
def is_any_3_corner(cartella, result):
    ...
    try:
        ...
        return sum(num in result or (idx == 12 and num == cartella[12]) for idx, num in enumerate(corners)) >= 3
    except Exception as e:
...
def is_all_4_corners(cartella, result):
    ...
    try:
        ...
        return all(num in result or (idx == 12 and num == cartella[12]) for idx, num in enumerate(corners))
    except Exception as e:
        print(f"Error in is_all_4_corners: {e}")
        return False
def is_any_2_lines(cartella, result):
    """Check if any 2 lines in the cartella are in the result, with a free middle space."""
    try:
        rows = [cartella[i:i + 5] for i in range(0, len(cartella), 5)]
        print(f"Checking Any 2 Lines for cartella rows: {rows}, result: {result}")
        return sum(all(num in result or (idx == 12 and num == cartella[12]) for idx, num in enumerate(row)) for row in rows) >= 2
    except Exception as e:
        print(f"Error in is_any_2_lines: {e}")
        return False
def is_any_3_lines(cartella, result):
    """Check if any 3 lines in the cartella are in the result, with a free middle space."""
    try:
        rows = [cartella[i:i + 5] for i in range(0, len(cartella), 5)]
        print(f"Checking Any 3 Lines for cartella rows: {rows}, result: {result}")
        return sum(all(num in result or (idx == 12 and num == cartella[12]) for idx, num in enumerate(row)) for row in rows) >= 3
    except Exception as e:
        print(f"Error in is_any_3_lines: {e}")
        return False
